...
| `jobs` | cada análise enviada: `type`, `status`, `stage`, `result` (JSONB), `error_message`. `stage_data.revisao` guarda o corretor de leitura: os trechos levantados, o que o usuário corrigiu e o desfecho. Os recortes em base64 só existem ali enquanto o job está em `awaiting_review` — fechada a revisão, saem do banco |
