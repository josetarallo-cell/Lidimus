...
- **Carimbo da certidão eletrônica**: a certidão do ONR sai carimbada na diagonal em cada página, e o Document AI transcreve o carimbo intercalado no conteúdo — "ESCREVENTE AUTORIZADO Cavalhieri" chegava à análise picotado por "PARA SIMPLES CONSU" / "UTORIZADO" / "NAO VALE COMO CERTIDAU". O índice de tokens traz o desvio angular de cada palavra em relação ao corpo da página (medido no `lidimus-OCR` sobre a população inteira de tokens, não sobre o índice filtrado) e o callback remove do texto o que destoa. As salvaguardas estão em `packages/revisao/carimbo.ts`: página com pouco texto fica intacta, página com mais de 30% de girado fica intacta (a medição é que está errada), remoção acima de 20% do documento é abortada, e o texto original fica em `stage_data.ocr.texto_ocr_bruto` com o que saiu em `carimbo_removido`.
